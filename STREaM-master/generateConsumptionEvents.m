        ...
        case 1
            for dayID = 1: param.H
                ...
                for eventID = 1:numEvents
                    ...
                    event_1=event;
                    event_1 = event_1(2:end-1);
                    
                    posPositions = find(event_1>0);
                    % Resizing signature length
                    while length(posPositions) > durations(eventID)
                        position=randi(length(posPositions),1);
                        event_1(posPositions(position)) = [];
                        posPositions = find(event_1>0);
                    end
                    
                    while length(posPositions) < durations(eventID)
                        position=randi(length(posPositions),1);
                        event_1 = [event_1(1:posPositions(position)), event_1(posPositions(position):end)];
                        posPositions = find(event_1>0);
                    end
                    
                    % Resizing signature volume
                    eventVolume = sum(event_1);
                    
                    volumeDifference = eventVolume - volumes(eventID);
                    if abs(volumeDifference)<= volumes(eventID) +volumes(eventID)*0.75
                        if volumeDifference > 0 % Signature should be lowered
                            coeffProp = event_1./eventVolume;
                            coeffProp(isnan(coeffProp)) = 0;
                        else % Signature should be increased
                            coeffProp = (event_1>0)/length(posPositions);
                            coeffProp(isnan(coeffProp)) = 0;
                        end
                        event = event_1 - volumeDifference.*coeffProp;
                        if sum((event_1<0))> 0
                            disp('error');
                        end
                    else
